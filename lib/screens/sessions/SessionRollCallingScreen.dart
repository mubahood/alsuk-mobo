...
class SessionRollCallingScreenState extends State<SessionRollCallingScreen> {
  ...
  Future<void> addToPresent(int id) async {
    if (!item.presentMembers.contains(id)) {
      item.presentMembers.add(id);
    }
  ...
  Future<void> addToAbsent(int id) async {
    if (!item.absentMembers.contains(id)) {
      item.absentMembers.add(id);
    }
