...
class EventModel {
  ...
  static Future<List<EventModel>> getLocalData({String where = "1"}) async {
    List<EventModel> data = [];
    if (!(await EventModel.initTable())) {
    ...
    List<Map> maps = await db.query(EventModel.tableName, where: where);

    if (maps.isEmpty) {
      return data;
    }
    List.generate(maps.length, (i) {
      data.add(EventModel.fromJson(maps[i]));
    });
  ...
  static Future<List<EventModel>> getItems({String where = '1'}) async {
    List<EventModel> data = await getLocalData(where: where);
    if (data.isEmpty) {
      await EventModel.getOnlineItems();
      data = await getLocalData(where: where);
    } else {
      data = await getLocalData(where: where);
      EventModel.getOnlineItems();
    }
    data.sort((a, b) => b.id.compareTo(a.id));
    return data;
  }

  static Future<List<EventModel>> getOnlineItems() async {
    List<EventModel> data = [];

    RespondModel resp =
    RespondModel(await Utils.http_get(EventModel.endPoint, {}));
    ...
    if (resp.data.runtimeType.toString().contains('List')) {
      ...
      await db.transaction((txn) async {
        ...
        for (var x in resp.data) {
          EventModel sub = EventModel.fromJson(x);
          try {
  ...
  static Future<bool> initTable() async {
    ...
    String sql = "CREATE TABLE  IF NOT EXISTS  ${EventModel.tableName} (  "
