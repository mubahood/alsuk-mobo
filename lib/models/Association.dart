...
class Association {
  static String endPoint = "associations";
  static String tableName = "associations";
  ...
  static fromJson(dynamic m) {
    ...
    obj.members = Utils.to_str(m['members'],'');
    obj.website = Utils.to_str(m['website'],'');
    obj.phone_number_2 = Utils.to_str(m['phone_number_2'],'');
    obj.vision = Utils.to_str(m['vision'],'');
    obj.mission = Utils.to_str(m['mission'],'');
  ...
  static Future<List<Association>> getLocalData({String where = "1"}) async {
    List<Association> data = [];
    if (!(await Association.initTable())) {
    ...
    List<Map> maps = await db.query(Association.tableName, where: where);

    if (maps.isEmpty) {
      return data;
    }
    List.generate(maps.length, (i) {
      data.add(Association.fromJson(maps[i]));
    });
  ...
  static Future<List<Association>> getItems({String where = '1'}) async {
    List<Association> data = await getLocalData(where: where);
    if (data.isEmpty) {
      await Association.getOnlineItems();
      data = await getLocalData(where: where);
    } else {
      data = await getLocalData(where: where);
      Association.getOnlineItems();
    }
    data.sort((a, b) => b.id.compareTo(a.id));
    return data;
  }

  static Future<List<Association>> getOnlineItems() async {
    List<Association> data = [];

    RespondModel resp =
    RespondModel(await Utils.http_get(Association.endPoint, {}));
    ...
    if (resp.data.runtimeType.toString().contains('List')) {
      ...
      await db.transaction((txn) async {
        ...
        for (var x in resp.data) {
          Association sub = Association.fromJson(x);
          try {
  ...
  static Future<bool> initTable() async {
    ...
    String sql = "CREATE TABLE  IF NOT EXISTS  associations (  "
