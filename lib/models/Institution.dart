...
class Institution {
  static String endPoint = "institutions";
  static String tableName = "institutions";
  ...
  static Future<List<Institution>> getLocalData({String where = "1"}) async {
    List<Institution> data = [];
    if (!(await Institution.initTable())) {
    ...
    List<Map> maps = await db.query(Institution.tableName, where: where);

    if (maps.isEmpty) {
      return data;
    }
    List.generate(maps.length, (i) {
      data.add(Institution.fromJson(maps[i]));
    });
  ...
  static Future<List<Institution>> getItems({String where = '1'}) async {
    List<Institution> data = await getLocalData(where: where);
    if (data.isEmpty) {
      await Institution.getOnlineItems();
      data = await getLocalData(where: where);
    } else {
      data = await getLocalData(where: where);
      Institution.getOnlineItems();
    }
    data.sort((a, b) => b.id.compareTo(a.id));
    return data;
  }

  static Future<List<Institution>> getOnlineItems() async {
    List<Institution> data = [];

    RespondModel resp =
    RespondModel(await Utils.http_get(Institution.endPoint, {}));
    ...
    if (resp.data.runtimeType.toString().contains('List')) {
      ...
      await db.transaction((txn) async {
        ...
        for (var x in resp.data) {
          Institution sub = Institution.fromJson(x);
          try {
