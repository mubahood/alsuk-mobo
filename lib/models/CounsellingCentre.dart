...
class CounsellingCentre {
  static String endPoint = "counselling-centres";
  static String tableName = "counselling_centres";
  ...
  static fromJson(dynamic m) {
    CounsellingCentre obj = CounsellingCentre();
    if (m == null) {
      return obj;
    }
  ...
  static Future<List<CounsellingCentre>> getLocalData({String where = "1"}) async {
    List<CounsellingCentre> data = [];
    if (!(await CounsellingCentre.initTable())) {
    ...
    List<Map> maps = await db.query(CounsellingCentre.tableName, where: where);

    if (maps.isEmpty) {
      return data;
    }
    List.generate(maps.length, (i) {
      data.add(CounsellingCentre.fromJson(maps[i]));
    });
  ...
  static Future<List<CounsellingCentre>> getItems({String where = '1'}) async {
    List<CounsellingCentre> data = await getLocalData(where: where);
    if (data.isEmpty) {
      await CounsellingCentre.getOnlineItems();
      data = await getLocalData(where: where);
    } else {
      data = await getLocalData(where: where);
      CounsellingCentre.getOnlineItems();
    }
    data.sort((a, b) => b.id.compareTo(a.id));
    return data;
  }

  static Future<List<CounsellingCentre>> getOnlineItems() async {
    List<CounsellingCentre> data = [];

    RespondModel resp =
    RespondModel(await Utils.http_get(CounsellingCentre.endPoint, {}));
    ...
    if (resp.data.runtimeType.toString().contains('List')) {
      if (await Utils.is_connected()) {
        await CounsellingCentre.deleteAll();
      }
      ...
      await db.transaction((txn) async {
        ...
        for (var x in resp.data) {
          CounsellingCentre sub = CounsellingCentre.fromJson(x);
          try {
  ...
  static Future<bool> initTable() async {
    ...
    String sql = "CREATE TABLE  IF NOT EXISTS  ${CounsellingCentre.tableName} (  "
